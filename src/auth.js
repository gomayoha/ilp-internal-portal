...
async function session(){const {data,error}=await client.auth.getSession();if(error)throw error;const user=data.session?.user;if(!user)return {role:'viewer',email:'visitor'};const permitted=check(await client.rpc('is_portal_admin'));return {role:permitted?'admin':'viewer',email:user.email};}
async function content(){const [directory,entries]=await Promise.all([client.from('portal_directory').select('people,prizes').eq('id',1).single(),client.from('portal_entries').select('*').order('created_at',{ascending:false})]);const seed=check(directory),rows=check(entries);return {...seed,...Object.fromEntries(['news','notifications','documents','pictures'].map(type=>[type,rows.filter(r=>r.type===type).map(r=>({...r,originalFilename:r.original_filename}))]))};}
window.PortalAuth={
...
window.PortalData={
 mediaUrl(relative){if(!/^\/media\/[a-zA-Z0-9_./-]+$/.test(relative)||relative.includes('..'))throw Error('Invalid file link.');return client.storage.from(bucket).getPublicUrl(relative.slice(7)).data.publicUrl;},
 async api(path,opts={}){
