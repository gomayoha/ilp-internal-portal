...
for(const file of files){const bytes=await fs.readFile(path.join(root,file));if(bytes.length>10485760)throw Error('A source file exceeds the 10 MB limit.');const {error}=await client.storage.from('portal-media').upload(file,bytes,{contentType:mime[path.extname(file)],upsert:false,cacheControl:'60'});if(error&&String(error.statusCode)!=='409')throw Error(error.message);}
